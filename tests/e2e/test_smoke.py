...
# ---- F-015: full-bleed canvas, overlay panels (backlog N12) --------------
# Ported from docs/design-explorations/canvas-frame.html.

#: The three widths the frame must hold at, per the task's C1/C5.
_FRAME_WIDTHS = (1440, 1024, 768)


def _set_state(page, region_id: str, toggle_id: str, state: str) -> None:
    """Click ``toggle_id`` if ``region_id`` is not already in ``state``."""
    if page.locator(f"#{region_id}").get_attribute("data-state") != state:
            ...
            for panel_state in ("expanded", "collapsed"):
                ...
                for drawer_state in ("expanded", "collapsed"):
                    ...
                    assert scroll_width == client_width, (
                        width, palette_state, panel_state, drawer_state, scroll_width, client_width,
                    )
