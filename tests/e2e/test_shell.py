...
def test_mission_panel_shows_m1_not_a_later_mission(index: LoadedPage) -> None:
    ...
    Goes red if the M-3 demo copy comes back. C3: only M-1 exists, so both
    pager buttons are disabled -- there is nothing to page to."""
    page = index.page
    ...
    assert page.locator("#pager-back").is_disabled()
    assert page.locator("#pager-forward").is_disabled()
