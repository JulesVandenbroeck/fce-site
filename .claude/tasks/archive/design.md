# Design — completed task archive

Full entries for every merged design task, moved out of `.claude/tasks/design.md` so that
...
The tasks are **not done**; their live status is in `.claude/tasks/design.md`. Read
...
## D-008 — full entry, moved from the active list 2026-08-21 on merge
...
## D-013 — full entry, archived 2026-09-02 on merge
...
## D-002 — Design token foundation (MERGED #24, `72d2950`, 3 cycles + 1 re-spec)

The full active-list entry as it stood at merge, 2026-09-03. checks=10, final gate `0R / 0M / 1m`.
