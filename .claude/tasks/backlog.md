...
- **N35** — `store.py` (B-033 #73 F1-F3): `:175` dead `counts = None`; `:141-153` `purge_class` pre-checks `class_exists` on a second connection (check-then-act) → raise `ValueError` when `classes == 0` after the deletes; `:130` `ORDER BY completed_at` → `ORDER BY rowid`. Backend, trivial. **Folded into B-034.**
