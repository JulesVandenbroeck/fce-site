...
- _(CLOSED by B-034 #78 + B-035 #77)_ **N38** — B-032 #75 F1-F3: `graph.py:416-419` card-gating half of C3 has no test (add `allowed_cards=("Multiplicity",)` → `GraphError` with `node_id` to the C3 test); `routes/api.py:90` dead `if mission else None`; `jobs.py:60-66` 7-line comment → 1. Backend, trivial. **Folded into B-034.**
- **N39** — `tests/test_progress.py:114` (B-034 #78 F10): comment opens with review-history label `F7:` → drop the prefix. Backend, trivial.
