...
- **[CLOSED by B-025 #49] B-018 fixture / F-008 readability** — the fixture's MC weights are full-sample production normalisations while only a 2000-event slice is kept, so weighted MC is ~6 events against ~712 raw pseudo-data. The physics is right and the shapes agree, but `yMax = max(stack, data)` renders the MC stack as a ~1px sliver and pegs every populated ratio bin at the panel clip: correct and unreadable. Needs either a scaled fixture weight or a demo-time normalisation choice. **Affects what the M3 checkpoint demo looks like.** Found 2026-09-16 on PR #47.

- **D-016 F1 (PR #48)** — C1's `elementFromPoint` check passed with the old row layout restored and the canvas cut off. Real guard: `.canvas-wrap` left ≥ `.canvas-region` left and `#hist-svg` top ≥ `.canvas-wrap` bottom, folded into `test_figure_holds_fixed_size_at_every_width` (frontend-owned e2e).
